...
extern int nrn_get_mechtype();
 ...
 "setdata_K_C_1D", _hoc_setdata,
 "rates_K_C_1D", _hoc_rates,
 0, 0
};
...
#define usetable usetable_K_C_1D
 double usetable = 1;
 ...
 "usetable_K_C_1D", &usetable_K_C_1D,
 0,0
};
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 5);
 	...
 	ivoc_help("help ?1 K_C_1D /cygdrive/c/Documents and Settings/Saci/Desktop/NEURON 7.2/scppinput/K_C_1D.mod\n");
...
static int _reset;
static char *modelname = "Channel: K_C_1D";
...
static int  _f_rates ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
  ...
  _ltemp_adj_X = 1.0 ;
   _lv = _lv * 0.0010 ;
   if ( _lv < - 0.015 ) {
     _lalpha = ( exp ( 53.872 * ( _lv + 0.065 ) - 0.66835 ) ) / 0.018975 ;
     }
   else {
     _lalpha = 2000.0 * ( exp ( ( ( - 0.065 ) + 0.0065 - _lv ) / 0.027 ) ) ;
     }
   _lalpha = _lalpha * 0.0010 ;
   _lv = _lv * 1000.0 ;
   _lv = _lv * 0.0010 ;
   if ( _lv < - 0.015 ) {
     _lbeta = 2000.0 * ( exp ( ( ( - 0.065 ) + 0.0065 - _lv ) / 0.027 ) ) - ( exp ( 53.872 * ( _lv + 0.065 ) - 0.66835 ) ) / 0.018975 ;
     }
   else {
     _lbeta = 0.0 ;
     }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   double _lg_factor , _lconc ;
 gion = gmax * ( pow( ( 1.0 * X ) , 1.0 ) ) ;
   _lconc = cai / 1.0 ;
   if ( _lconc < 250.0 ) {
     _lg_factor = _lconc / 250.0 ;
     }
   else {
     _lg_factor = 1.0 ;
     }
   gion = gion * _lg_factor ;
   ik = gion * ( v - ek ) ;
   }
 _current += ik;
