...
def to_bpo_param(attrs):
    ...
    for k,v in attrs.items():
        ...
        #import pdb
        #pdb.set_trace()
        #p.lower_bound = temp[0]
        #p.upper_bound = temp[1]
        lop[k] = p

        #print(k)
        #print(lop[k].lower_bound)
        #print(lop[k].upper_bound)
        #print('passed')
...
MATNEURON = {'vr':-65.0,'vt':-55.0,'a1':10, 'a2':2, 'b':0.001, 'w':5, 'R':10, 'tm':10, 't1':10, 't2':200, 'tv':5, 'tref':2}
MATNEURON = {k:(np.mean(v)-np.abs(np.mean(v)*0.125),np.mean(v)+np.abs(np.mean(v))*0.125) for k,v in MATNEURON.items()}
