...
def searcher(f,rh_param,vms):
    ...
    while boolean==False and cnt<6:
        ...
        if type(rh_param[1]) is float:
            ...
            returned_list=[]
            vmslist = [vms for i in range(0,len(rh_param[1])) ]

            returned_list = list(dview.map(check_current,rh_param[1],vmslist))
            print(vmslist)
            #d={}
            assert vms!=None
            ...
            returned_list = list(dview.map(check_current,vms.steps,vmslist))
            for v in returned_list:
                vms.lookup.update(v.lookup)
            boolean,vms=check_fix_range(vms)
            if boolean:
