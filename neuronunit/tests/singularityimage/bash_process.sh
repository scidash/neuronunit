...
if [ -f "singularity/*.img" ] ; then rm *.img;  fi
...
sudo singularity bootstrap singularityimage/pnp*.img nu.def
#Add the required files.
sudo singularity exec singularityimage/pnp*.img python 

sudo rsync singularityimage/pnp*.img rjjarvis@comet.sdsc.xsede.org:~/
