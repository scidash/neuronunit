...
def check_match_front(dtc0,dtcpop,figname = None):
    ...
    for v in vms:
        plt.plot(v.times, v.magnitude,label="other solutions",c='grey')
    plt.ylabel('V (mV)')
...
def allen_wave_predictions(dtc,thirty=True):
    if thirty:
        vm30 = dtc.vm30
        #dtc = DataTC()
        try:
        ...
        try:
            ...
            ephys = EphysSweepFeatureExtractor(t=np.array(t),v=np.array(v),filter=8)#,\
        ...
        try:
            ...
            pass
        v = [float(v*1000.0) for v in vm15.magnitude]
        t = [float(t) for t in vm15.times]
        try:
            #spks = ft.detect_putative_spikes(np.array(v),np.array(t))
    ...
    if not 'spikes' in ephys_dict.keys() or ephys_dict['avg_rate'] == 0:
        dtc.scores = None
        dtc.preds = {}
        print('early return how often?')
        print(dtc.rheobase)
        two = inject_and_not_plot_model(dtc)
        print('mean:',np.mean(two[0]))
        #plt.show()
        return dtc, None
    else:
        prediction = {}
...
def dask_memory_friendly(self, f, x):
    """
    Similar to dask-map, but a memory friendly version for big data.
    Uses lazy evaluation as opposed to storing everything in RAM.
    """
    x = [delayed(f(j)) for j in x]
    return compute(*x)
...
def nuunit_allen_evaluation(dtc):
    ...
    if hasattr(dtc,'vtest'):
        ...
        try:
            ...
            #[ dtc.pre_obs['spike_count']['mean'], dtc.pre_obs['spike_count']['mean']]
            observation_spike = {'value': dtc.tsr}

        #import pdb
        #pdb.set_trace()
        if not str('GLIF') in str(dtc.backend):
            scs = SpikeCountSearch(observation_spike)
            print(observation_spike)
            model = new_model(dtc)
            assert model is not None
...
def nuunit_dm_evaluation(dtc):
    ...
    model.set_attrs(dtc.attrs)
    try:
        params = [v for v in dtc.tests.values()][0].params
    except:
        params = dtc.tests[0].params

    current = params['injected_square_current']
    current['delay'] = 1000*pq.ms
    current['duration'] = 2000*pq.ms
    ...
    dm_test_features,dm_results_regular = DMTNMLO.runTest()
    dtc.dm_test_features = None
    dtc.dm_results_regular = None
    dtc.dm_test_features = dm_test_features
    dtc.dm_results_regular = dm_results_regular
    ''''
    dtc.AP1DelayMeanTest = None
    dtc.AP1DelayMeanTest = dm_test_features['AP1DelayMeanTest_1.5x']
    dtc.InputResistanceTest = dm_test_features['InputResistanceTest_1.5x']
...
def efel_evaluation(dtc):
    ...
    ephys.process_spikes()
    ephys_dict = ephys.as_dict()
    spks = ephys_dict['spikes']#ft.detect_putative_spikes(np.array(v),np.array(t))
    #dtc.spks = None
    dtc.spks = spks
    #model = dtc.dtc_to_model()
    #model.vm30 = dtc.vm30
    ...
    trace3 = {'T': [float(t) for t in dtc.vm30.times.rescale('ms')], 'V': [float(v) for v in dtc.vm30.magnitude],
              'stimulus_current': [dtc.rheobase*3.0]}
    ...
    trace15 = {}
    trace15['T'] = [float(t) for t in dtc.vm15.times.rescale('ms')]
    trace15['V'] = [float(v) for v in dtc.vm15.magnitude]
    trace15['stim_start'] = [ trace15['T'][0] ]
    trace15['stimulus_current'] = [ dtc.rheobase*1.5 ]
    ...
    if len(threshold_detection(dtc.vm15, threshold=0)):
        threshold = float(np.max(dtc.vm15.magnitude)-0.5*np.abs(np.std(dtc.vm15.magnitude)))


    #efel_15 = efel.getMeanFeatureValues(traces15,list(efel.getFeatureNames()))#
    else:
        threshold = float(np.max(dtc.vm15.magnitude)-0.2*np.abs(np.std(dtc.vm15.magnitude)))
    efel.setThreshold(threshold)
    if np.min(dtc.vm15.magnitude)<0:
        efel_15 = efel.getMeanFeatureValues(traces15,list(efel.getFeatureNames()))
    else:
        efel_15 = None
    efel.reset()
    if len(threshold_detection(dtc.vm30, threshold=0)):
        threshold = float(np.max(dtc.vm30.magnitude)-0.5*np.abs(np.std(dtc.vm30.magnitude)))
    else:
        threshold = float(np.max(dtc.vm30.magnitude)-0.2*np.abs(np.std(dtc.vm30.magnitude)))
        efel.setThreshold(threshold)
    if np.min(dtc.vm30.magnitude)<0:
...
class OptMan():
    ...
    def simulate_data(self,
        ...
        if self.protocol['allen']:
            ...
            #from neuronunit.optimisation.optimisations import run_ga
            #MU = 10, NGEN = 10
            #ga_out, DO = run_ga(ranges,NGEN,new_tests,free_params=rp.keys(), MU = MU, backend=backend, protocol={'allen':True,'elephant':False,'tsr':[target_spikes,target_spikes]})
            #ga_converged = [ p for p in ga_out['pf'] ]
            #ga_out,ga_converged,test_origin_target,new_tests
            #return ga_out,ga_converged,test_origin_target,new_tests,self.protocol['tsr']
            print('Random simulated data tests made')
        ...
        if self.protocol['allen']:
            ...
            while target_current is None or not hasattr(dtc,'spike_cnt'):# is None:
                ...
                try:
                    ...
                    scs = make_simulated_observations(observation_range)
                    model = new_model(dtc)
            ...
            simulated_tests = dtc.pre_obs = dtc.preds
            return simulated_tests, dtc
    #@timer
