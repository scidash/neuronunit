...
#get_ipython().system('pip install igor')
...
def plot_rheobase(all_sweeps):

    for j,i in enumerate(all_sweeps):

        if data[i].units != pq.pA:
            threshold = spike_functions.threshold_detection(data[i])
    ...
    for j,i in enumerate(all_sweeps):
        ...
        if data[i].units != pq.pA:
            ...
            if len(threshold) == np.min(threshes):
                plot_data(data[i])
...
data_ids = list_curated_data()[0:10]

for di in data_ids:

    data = get_curated_data(di)
    all_sweeps = list_sweeps(di)
