...
class NeuroElectroTestCase(unittest.TestCase):
    ...
    @unittest.skipUnless(
        neuroelectro.is_neuroelectro_up(), "NeuroElectro URL is not responsive"
    )
...
class BlueBrainTestCase(NotebookTools, unittest.TestCase):
    ...
    @unittest.skipUnless(bbp.is_bbp_up(), "Blue Brain URL is not responsive")
    def test_bluebrain(self):
        self.do_notebook("bbp")


class AIBSTestCase(unittest.TestCase):
    @unittest.skipUnless(aibs.is_aibs_up(), "AIBS URL is not responsive")
    def test_aibs(self):
        dataset_id = (
            354190013  # Internal ID that AIBS uses for a particular Scnn1a-Tg2-Cre
        )
