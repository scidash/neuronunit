...
class NEURONBackend(Backend):
    def __init__(self,nml_file_path,name=None,attrs=None):
        '''
    ...
    def load_model(self):        
        ...
        f=pynml.run_lems_with_jneuroml_neuron(self.nml_file, 
                                          skip_run=False,
                                          nogui=False, 
    ...
    def inject_square_current(self,current):
        ...
        current['amplitude'] = re.sub('\ pA$', '', str(current['amplitude']))

        amps=float(current['amplitude'])/1000.0
        evalstring='self.neuron.hoc.execute("explicitInput_RS_IextRS_pop0.'+str('amplitude')+'='+str(amps)+'")'
        print(evalstring) 
        eval(evalstring)
        #evalstring='self.neuron.hoc.execute("explicitInput_RS_IextRS_pop0.'+str('duration')+'='+str(1000.0)+'")'
        #print(evalstring) 
        eval(evalstring)
