...
#@jit#(nopython=True)
@jit(fastmath=True)
def get_vm_matlab_one(C=89.7960714285714,
...
class IZHIBackend(Backend):
    ...
    def init_backend(self, attrs=None,DTC=None,
        ...
        self.default_attrs = {'C':89.7960714285714, 'a':0.01, 'b':15, 'c':-60, 'd':10, 'k':1.6, 'vPeak':(86.364525297619-65.2261863636364), 'vr':-65.2261863636364, 'vt':-50, 'celltype':1}
    ...
    def get_membrane_potential(self):
        ...
        if type(self.vM) is type(None):
            ...
            #print(everything)
            #print(everything.items())

            v = get_vm_matlab(**everything)
    ...
    def inject_square_current(self, current):#, section = None, debug=False):
        ...
        #import asciiplotlib as apl
        #fig = apl.figure()
        #ig.plot([float(f) for f in self.vM.times], [float(f) for f in self.vM], width=100, height=20)
        #fig.show()
